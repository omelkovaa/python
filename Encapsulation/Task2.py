...
в случае победы героя в битве и удалить ранг в случае поражения.
"""
...
class Hero:
    ...
    def fight(self, enemy):
        while self.power > 0:
            enemy.health -= 2  # 200 для другого результата
            self.power -= 10
        if enemy.health < 0:
            self.set_rank('Победитель')
            print(self.get_rank())
        else:
            self.del_rank()
            print('Плохо, без ранга, проигрыш')


def check():
    hero1 = Hero(100, 500, 'живой')
    hero2 = Hero(100, 1000, 'убитый')
    hero1.fight(hero2)
